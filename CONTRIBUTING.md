...
First off, thank you for considering contributing to mod-xpon-prpl and the [Ambiorix](https://gitlab.com/prpl-foundation/components/ambiorix) project. It's people like you that makes [Ambiorix](https://gitlab.com/prpl-foundation/components/ambiorix) such a great set of tools and libraries.
...
mod-xpon-prpl is a part of the [Ambiorix](https://gitlab.com/prpl-foundation/components/ambiorix) project. All parts of [Ambiorix](https://gitlab.com/prpl-foundation/components/ambiorix) are open source projects and we love to receive contributions from our community — you!

There are many ways to contribute, from writing tutorials or blog posts, improving the documentation, submitting bug reports and feature requests or writing code which can be incorporated into mod-xpon-prpl or the [Ambiorix](https://gitlab.com/prpl-foundation/components/ambiorix) project itself.
...
# Contributions to mod-xpon-prpl

## Getting started

Fork the main (called `upstream`) repository to a private repository. 

After forking you have your own copy of the repository, available at `https://gitlab.com/<USER>/mod-xpon-prpl` where `<USER>` is your user name on GitLab.

Clone your fork on your computer (replace `<USER>` by your gitlab user name):

```bash
git clone git@gitlab.com:<USER>/mod-xpon-prpl.git
```

Add `upstream` as a remote called `upstream`:

```bash
git remote add upstream git@gitlab.com:prpl-foundation/components/core/modules/mod-xpon-prpl.git
```
...
If you find yourself wishing for a feature that doesn't exist in Ambiorix (in general) or mod-xpon-prpl (specific), you are probably not alone. Open an issue on our issues list on GitLab which describes the feature you would like to see, why you need it, and how it should work.
